...
There are a few bugs in the game currently, however most of these bugs is simply a lack of function of components rather than something that may reduce the existing game experience. There is still currently a small bug that may be resolved by submission time regarding the orign of rotation for the health bar, resulting in the internal and external rectangles gradually becoming de-synchronized in their rotation. Another bug is that the sprite for player 2 changes back to being the sprite for player 1 upon death in order for the death animation to work properly.
